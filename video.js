...
(async () => {
    ...
    for (let i = 0; i < numOfAcc; i++) {
        ...
        const email = (`${firstName}${lastName}${Math.floor(Math.random() * 9999)}@mail.ru`).toLowerCase();
        const password = "vidio1tahun";
        ...
        if (register?.auth?.email) {
            ...
            const partnerReq = await request('https://www.vidio.com/api/partner/auth', 'POST', headers, `{"serial_number":"${tclId}","partner_agent":"le"}`);
            ...
            if (partnerReq?.auth?.email) {
                ...
                if (login?.auth?.email) {
                    ...
                    if (subscription?.has_active_subscription === true) {
                        ...
                        console.log(colors.green(`Email: ${email} | Password: ${password} | Offer: ${subscription.has_active_subscription}`))
                        fs.appendFileSync('result.txt', `${email}|${password}\n`);
                    } else {
                        console.log(colors.red(`Email: ${email} | Password: ${password} | Offer: ${subscription.has_active_subscription}`))
                    }

                } else {
                    console.log(colors.red(`Email: ${email} | Password: ${password} | Error: ${login.error}`))
                }

            } else {
                console.log(colors.red(`Email: ${email} | Password: ${password} | Error: ${partnerReq.error}`))
            }
        } else {
            console.log(colors.red(`Email: ${email} | Password: ${password} | Error: ${register.error}`))
        }
        await delay(2000);
    }
